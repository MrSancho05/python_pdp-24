...
def access(x):
    if x == 1:
        userPassword = input('Parolni kiriting: ')
        inpPassword(userPassword)
    elif x == 2:
        print('ru')
    elif x == 3:
        print('en')
...
def chooseLanguage():
    print(f'Kirish\n1.Uzbekcha\n2.Руский\n3.English')
    x = int(input('Menyudan birini tanlang: '))
    return x

# parol kiritish qismi
def inpPassword(password):
    savedPassword = '1234'
    if password == savedPassword:
        # Bosh menyu chaqirilishi
        chooseOption()
    else: print('Parol xato kiritilgan')
...
# balansni ko'rsatish
def showAmountOfMoney():
    print(f"Balansingizda {cash()} so'm mablag' bor")
    print('1. Orqaga')
    stop = int(input('Menyudan birini tanlang: '))
    if stop == 1:
        chooseOption()
    else:
        pass
...
def getMoney():
    ...
    if x == 1:
        ...
        anotherChash = int('Qancha yechmoqchisiz: ')
        if anotherChash < cash2:
            cash2 -= anotherChash
        else: 
